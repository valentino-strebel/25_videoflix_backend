...
def test_activate_flow(client: APIClient):
    user = create_user()
    from authentication.tokens import activation_token_generator
    ...
    url = reverse("authentication:activate", args=[uidb64, token])
    res = client.get(url)
...
def test_password_confirm_flow(client: APIClient):
    ...
    user.is_active = True
    user.save()
    from authentication.tokens import password_reset_token_generator
    from django.utils.http import urlsafe_base64_encode
    from django.utils.encoding import force_bytes

    uidb64 = urlsafe_base64_encode(force_bytes(user.pk))
    token = password_reset_token_generator.make_token(user)
    url = reverse("authentication:password_confirm", args=[uidb64, token])
